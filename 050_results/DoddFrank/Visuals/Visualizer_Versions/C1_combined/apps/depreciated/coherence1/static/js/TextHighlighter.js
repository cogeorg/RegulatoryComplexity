...
(function (global) {
    ...
    TextHighlighter.prototype.doHighlight = function (keepRange) {
        ...
        object_aux.setReady(false);

        function wait() {
            var interval = setInterval(function () {
                if (object_aux.options.ready) {
                     clearInterval(interval);
                     if (object_aux.options.click) {
                      timestamp = +new Date();
                      wrapper = TextHighlighter.createWrapper(object_aux.options);
                      wrapper.setAttribute(TIMESTAMP_ATTR, timestamp);
                      createdHighlights = object_aux.highlightRange(range, wrapper);
                      normalizedHighlights = object_aux.normalizeHighlights(createdHighlights);
                      object_aux.options.onAfterHighlight(range, normalizedHighlights, timestamp);
                      }

                     if (!keepRange) {
                         dom(object_aux.el).removeAllRanges();
                      }
    ...
    /**
     * Sets ready when a color is selected.
     * @param {string} color - valid CSS color.
     * @memberof TextHighlighter
     */
    TextHighlighter.prototype.setReady = function (ready) {
     ...
     * @memberof TextHighlighter
     */
    TextHighlighter.prototype.setClick = function (click) {
     ...
     * @memberof TextHighlighter
     */
    TextHighlighter.prototype.doHighlightFind = function (keepRange) {
        var range = dom(this.el).getRange(),
            wrapper,
            createdHighlights,
            normalizedHighlights,
        ...
        var innertext = container.parentNode.textContent;
        var type_node = container.parentNode.nodeName.toLowerCase();
        ...
        if (type_node != "span" || (this.getColor() == "white" && innertext==range)) {
